...
runtime stage), so it carries no `tsx`/`drizzle-kit` to run migrations with.
`db` is also published on `localhost:5432` — run migrations from the host
instead, against the same database:
